(ns regulation.actor
  "RegulatoryActor — the ISIC Rev.5 8412 health care and social services
  regulatory administration actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  regulatory or compliance operation request (intake → advise → govern →
  ...
  The unconditional invariant: the RegulatoryAdvisor can never directly
  commit a record or dispatch a robot action the RegulatoryGovernor refuses
  — every commit-record! call is gated behind `:decide`. Additionally, any
  proposal touching license issuance, suspension, revocation, or binding
  regulatory determinations PERMANENTLY HOLDS (no path to approval)."
...
(defn build-graph
  "Build a compiled RegulatoryActor graph. `store` implements
...
(defn run-request!
  "Run one regulatory/compliance operation request to completion or
